...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    if self.activation is 'relu':
        ...
        z1 = np.clip(z1,-700,np.inf)
        hidden = np.log(1+np.exp(z1*(z1<700)))+z1*(z1>700)-np.log(2)*(z1>700)
    elif self.activation is 'sigmoid':
        z1 = np.clip(z1,-700,700)
    ...
    ###########################################################################
    L2_regularize_loss = 0.5*(np.linalg.norm(W1)**2 + np.linalg.norm(W2)**2)
    
    #numerical stabiliity of softmax
    scores_max = np.max(scores,axis=1,keepdims=True)
    e = np.exp(scores - scores_max)
    y_prob = (e+epsilon)/(np.sum(e, axis=1,keepdims=True)+epsilon)
    log_y_prob = np.log(y_prob)
                    
    softmax_loss = -np.sum(log_y_prob * np.eye(C)[y],axis=1,keepdims=True)
    loss = reg*L2_regularize_loss + np.mean(softmax_loss)
    ###########################################################################
    #                            END OF YOUR CODE
    ###########################################################################
    ...
    # output layer
    dsoftmax_loss = 1/N
    dscore = dsoftmax_loss*(y_prob - np.eye(C)[y])  # partial derivative of loss wrt. the logits (dL/dz)
    
    dW2 = np.transpose(hidden) @ dscore + reg*W2    # partial derivative of loss wrt. W2
    #print(f'dW2 :{dW2}')
    db2 = np.sum(dscore,axis=0,keepdims=True)     # and so on...
    #print(f'db2:{db2}')
    dhidden = dscore @ np.transpose(W2)
    #print(f'dhidden:{dhidden}')
    # hidden layer
    if self.activation is 'relu':
        dz1 = dhidden
        dz1[z1 <= 0] = 0
  ...
  def predict(self, X):
    ...
    y_pred = np.nanargmax(scores, axis=1)
    ###########################################################################
    #                              END OF YOUR CODE
    ###########################################################################
