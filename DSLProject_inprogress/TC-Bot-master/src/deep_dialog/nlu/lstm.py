...
class lstm(SeqToSeq):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.model['WLSTM'] = initWeights(input_size + hidden_size + 1, 4*hidden_size) #initialise the weight matrix for x_t, h_{t-1}
        # Hidden-Output Connections
        self.model['Wd'] = initWeights(hidden_size, output_size)*0.1 #initialise weights to h_t to o_t
        self.model['bd'] = np.zeros((1, output_size)) #initialise bias to h_t to o_t

        self.update = ['WLSTM', 'Wd', 'bd']  #list of update parameters 
        self.regularize = ['WLSTM', 'Wd'] #list of regularize parameters

        self.step_cache = {}
        
    """ Activation Function: Sigmoid, or tanh, or ReLu """
    def fwdPass(self, Xs, params, **kwargs):
        predict_mode = kwargs.get('predict_mode', False) #getting the predict mode status from **kwargs
        
        Ws = Xs['word_vectors'] #input. need to check the format of input??
        
        WLSTM = self.model['WLSTM'] 
        n, xd = Ws.shape # n = no. of words in the word vector
        
        #creating zero matrix for corresponding stages
        d = self.model['Wd'].shape[0] # size of hidden layer, fixed throughout
        Hin = np.zeros((n, WLSTM.shape[0])) # xt, ht-1, 
        Hout = np.zeros((n, d))
        IFOG = np.zeros((n, 4*d)) #IFOG
        IFOGf = np.zeros((n, 4*d)) # after nonlinearity bias
        Cellin = np.zeros((n, d)) 
        ...
        for t in xrange(n):
            ...
            Hin[t,0] = 1 # bias
            Hin[t, 1:1+xd] = Ws[t] #x_t
            Hin[t, 1+xd:] = prev #h_{t-1}
            
            # compute all gate activations. dots:
            IFOG[t] = Hin[t].dot(WLSTM)  #getting IFOG values
            
            IFOGf[t, :3*d] = 1/(1+np.exp(-IFOG[t, :3*d])) # sigmoids; these are three gates
            IFOGf[t, 3*d:] = np.tanh(IFOG[t, 3*d:]) # tanh for input value
            
            Cellin[t] = IFOGf[t, :d] * IFOGf[t, 3*d:] #I*G
            if t>0: Cellin[t] += IFOGf[t, d:2*d]*Cellin[t-1] # F*c_{t-1} + I*G
            
            Cellout[t] = np.tanh(Cellin[t])
            
            Hout[t] = IFOGf[t, 2*d:3*d] * Cellout[t] #O*tanh(F*c_{t-1} + I*G)

        Wd = self.model['Wd']
        bd = self.model['bd']
            
        Y = Hout.dot(Wd)+bd #output
